...
- The root `.htaccess` sends `Cache-Control: no-cache` for html/js/css so a deploy never pairs a new `index.html` with a cached old `script.js`. It needs `mod_headers`, which the compose `command` enables. The `?v=2` on `script.js`/`styles.css` in `index.html` only flushed copies cached before that header existed; it doesn't need bumping per deploy.
...
- `script.js` — all app logic as global functions/vars (no modules). Key pieces:
  ...
  - Clicking a player moves keyboard focus into its cross-origin iframe, which swallows key presses; `addKeyboardShortcuts()` takes focus back so shortcuts keep working: on window `blur` for Chrome, plus a 250 ms check because Firefox fires no `blur` event in this case.
  - Auto Mix (the `autoMix` object, bottom of the file): a 1 s monitor starts a crossfade `AUTO_MIX_FADE_SECONDS` before the audible deck ends. The other deck gets a video from `api/related.php` (via `loadDeck(..., true)`, since `playVideo()` right after `cueVideoById()` gets dropped) and `animateFader()` moves the crossfader using clock-based progress, so throttled background timers still finish on time. Picks are scored from videos related to the seed (weight 2) and the last 2 played, excluding everything already played. A player `onError` (e.g. embedding disabled) during a fade reverts to the previous deck and picks again. There is intentionally no length filter: long mixes are fine as background music.
...
- `api/` — PHP search proxy that keeps the API key server-side.
  ...
  - `invidious-search.php` — uses `invidious_get()` from `common.php`, which tries `INVIDIOUS_INSTANCE`, then `INVIDIOUS_FALLBACK_INSTANCES`, then API-enabled instances from `api.invidious.io` (cached 6h), up to `INVIDIOUS_MAX_ATTEMPTS`. Public instances often disable their API or vanish, so don't rely on hardcoded lists. Thumbnails are built from `i.ytimg.com` rather than taken from the instance.
  - `related.php` — related videos for Auto Mix: Invidious `/api/v1/videos/{id}` `recommendedVideos` (the YouTube Data API dropped related videos in 2023). The fallback is videos.list plus a search of the same channel (~101 quota units), and results are cached. Invidious gives `published` as a Unix timestamp in search results but as an ISO string in recommendations; `invidious_video_item()` handles both.
